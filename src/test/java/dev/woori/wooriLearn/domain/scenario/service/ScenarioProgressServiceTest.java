...
class ScenarioProgressServiceTest {
    ...
    @InjectMocks private ScenarioProgressService service;
    ...
    void advance_noQuizToNext() {
        // given: s1에는 퀴즈 없음
        ScenarioStep s1 = ScenarioStep.builder().id(101L).scenario(scenario).type(StepType.DIALOG).content("{\"a\":1}").build();
        ScenarioStep s2 = ScenarioStep.builder().id(102L).scenario(scenario).type(StepType.DIALOG).content("{\"b\":2}").build();
        s1 = ScenarioStep.builder().id(101L).scenario(scenario).type(StepType.DIALOG).content("{\"a\":1}").nextStep(s2).build();
    ...
    void advance_completed() {
        ...
        when(completedRepository.findByUserAndScenario(user, scenario)).thenReturn(Optional.empty());
